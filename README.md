...
**让 DSH 提交的命令脱离 DSH 进程独立运行：任务由 Windows 任务计划程序服务托管，关掉 DSH、关掉网页、甚至关掉终端窗口都不影响任务执行。任务退出时自动通知创建它的 agent，网页随时看实时输出，DSH 离线也能用独立 CLI/GUI 管理。**
...
| **实时输出面板** | 网页右下角「后台任务监控」浮动面板每秒刷新打印输出，可拖拽移动、▾/▸ 折叠、随主题换肤 |
| **完成自动通知** | 任务退出瞬间把退出码与日志路径推送给创建它的 agent 会话（fs.watch 事件驱动，亚秒级） |
...
- 任务退出后 `<workdir>\.dsh\bgjobs\<jobId>\exitcode.txt` 写入退出码，创建它的 agent 会话收到完成通知（含退出码与日志路径）；
- 网页右下角「后台任务监控」面板实时显示全部任务与输出：点任务行展开日志尾部，标题栏可拖拽，`▾` / `▸` 折叠面板。
...
> 离线提交的任务，DSH 恢复后会自动接管跟踪，并在创建会话存在时补发完成通知。
