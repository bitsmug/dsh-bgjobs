...
//   - 任务 bat 把每条命令输出重定向到日志，末尾写 exitcode 并自删任务；
...
/** 生成任务 bat：每条命令输出重定向到日志，末尾写 exitcode 并自删任务计划。 */
export function buildBat(job) {
  const lines = ['@echo off', 'cd /d "' + job.meta.workdir + '"']
  const cmds = String(job.meta.command).split(/\r?\n/).map((l) => l.trim()).filter((l) => l.length > 0)
  for (const c of cmds) lines.push(c + ' >> "' + job.meta.logPath + '" 2>&1')
  lines.push('set "bgrc=%errorlevel%"')
  lines.push('>> "' + job.meta.logPath + '" echo [BGJOB] exit code: %bgrc%')
  lines.push('> "' + job.meta.exitcodePath + '" echo %bgrc%')
  lines.push('schtasks /Delete /TN ' + job.meta.taskName + ' /F >nul 2>&1')
  return lines.join('\r\n') + '\r\n'
}
...
export function apply(ctx) {
  ...
  const notifyCreator = (job, exitCode) => {
    if (job.notified) return
    job.notified = true
    const sid = job.meta.createdBySession
    if (!sid) return
    ...
    try {
      agent.followup({
        id: 'msg-bgj-' + job.id,
        role: 'user',
        content: [{ type: 'text', text: '后台任务「' + job.meta.name + '」已结束（exit=' + exitCode + '）。日志：' + job.meta.logPath }],
        source: { kind: 'plugin', plugin: 'bgjobs' },
      })
    } catch (e) { /* 会话不可达时静默（job.json 已落盘终态） */ }
  }
  ...
  const submitJob = async (jobName, command, workdirRaw, createdBySession) => {
    ...
    const job = { id: jobId, meta, status: 'running', exitCode: undefined, pos: 0, tail: '', notified: false, decoder: new TextDecoder(), watch: undefined, checkTimer: undefined, lastCompletionCheck: 0 }
    try {
      ...
      for (const n of names) {
        try {
          ...
          registry.set(meta.id, {
            ...
            finishedAt: meta.finishedAt, notified: meta.status === 'done',
  ...
  const disposeRoutes = ctx.inject(['webServer'], (webCtx) => {
    const handler = (req, res) => {
      ...
      if (pathname === '/bgjobs/state') {
        ...
        res.end(JSON.stringify({ ok: true, jobs: Array.from(registry.values()).map(view) }))
        return
      }
      res.writeHead(404)
      res.end()
  ...
  const disposeSubmit = ctx.tools.register({
    name: 'bgjob_submit',
    description: '把命令提交为独立于 DSH 进程的后台任务：生成 bat（每条命令输出重定向到日志 + 末尾写 exitcode + 自删任务），经 schtasks 一次性任务交由任务计划程序服务托管，关 DSH/关终端不影响。网页「后台任务监控」面板实时显示输出，任务退出时自动通知创建它的 agent。command 支持多行逐行执行；workdir 必须是 DSH 工作区内的绝对路径（任务文件与日志写在 <workdir>/.dsh/bgjobs/<id>/ 下）。',
    presentCall: (args) => ({
      card: 'generic',
      title: '提交后台任务',
      kind: 'execute',
    ...
    parameters: {
      ...
      properties: {
        ...
        command: { type: 'string', description: '要执行的命令（可多行，逐行执行；bat 语法，如 for 循环变量用 %%i）' },
