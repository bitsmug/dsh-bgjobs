...
export function buildBgjobsGuidance() {
  return [
    ...
    '- 可选 wait：bgjob_submit / bgjob_submit_pwsh 传 `wait: <秒>`（1–600）会在提交成功后自动等待任务结束再返回结果（同 bgjob_wait 语义；超时返回 timedOut 快照，可再调 bgjob_wait 续等）。',
...
export function apply(ctx) {
      ...
      for (const ws of workspaces) {
        ...
        for (const n of names) {
          try {
            ...
            if (job) {
              if (job.status === 'done') await readLog(job)
              indexUpsert(job)
            }
          } catch (e) { /* 非任务目录 */ }
        }
  ...
  const disposeSubmit = ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      if (res.ok && waitSec > 0) return waitJobDone(res.jobId, waitSec * 1000)
      return res
    },
  })
