...
// ── bgjob_wait 轮询（指数退避，v0.1.50）──────────────────────────────
// 0–30s 每 250ms 一查（短任务响应快）→ 30–60s 500ms → ≥60s 封顶 1s，
// 长任务不高频空转。先把 elapsed 截断成档位再算指数（step ≤ WAIT_MAX_STEPS），
// 避免最长 600s 等待时（floor 达 20）做大数指数运算。
const WAIT_POLL_MS = 250
const WAIT_FAST_WINDOW_MS = 30_000
const WAIT_POLL_MAX_MS = 1000
const WAIT_MAX_STEPS = 2
...
export function apply(ctx) {
  ...
  const waitSnapshot = async (jobId) => {
    ...
    if (disk) return { found: true, done: disk.status === 'done', id: disk.id, name: disk.name, status: disk.status, exitCode: disk.exitCode, logPath: disk.logPath, tail: disk.tail }
    return null
  }
  ...
  const disposeWait = ctx.tools.register({
    ...
    async execute(args) {
      ...
      while (Date.now() - started < timeoutMs) {
        // 指数退避：0–30s 250ms → 30–60s 500ms → ≥60s 1s（上限）。
        // 先截断 elapsed 成档位再算指数（step ≤ WAIT_MAX_STEPS），避免长等待做大数指数运算。
        const step = Math.min(Math.floor((Date.now() - started) / WAIT_FAST_WINDOW_MS), WAIT_MAX_STEPS)
        await sleep(Math.min(WAIT_POLL_MAX_MS, WAIT_POLL_MS * 2 ** step))
