...
export function apply(ctx) {
   ...
   * 会话受限时更宽请求在 full access 开关关时经 ctx.approval 弹窗审批（镜像 dsh 升权流程）。
  ...
  const submitJob = async (jobName, command, workdirRaw, createdBySession, engine = 'bat', sandboxMode = 'off') => {
  ...
  const disposeSubmit = ctx.tools.register({
    name: 'bgjob_submit',
    description: '把命令提交为独立于 DSH 进程的后台任务：用户命令原样写入子 bat，经 schtasks 一次性任务交由任务计划程序服务托管（日志 UTF-8、末尾写 exitcode、自删任务计划），关 DSH/关终端不影响。网页「后台任务监控」面板实时显示输出，任务退出时自动通知创建它的 agent。command 支持多行与 for/if 块结构（bat 语法，循环变量用 %%i），原样写入子 bat 执行；workdir 必须是 DSH 工作区内的绝对路径（任务文件与日志写在 <workdir>/.dsh/bgjobs/<id>/ 下）。本工具（bat 引擎）任务恒为全权限、不支持 sandbox：若当前会话被 dsh 沙箱策略受限（bgjobs 面板 full access 开关关），提交会弹窗请求用户批准；需要受限后台任务请用 bgjob_submit_pwsh 的 sandbox 参数。',
    presentCall: (args) => ({
      card: 'generic',
      title: '提交后台任务',
      kind: 'execute',
    ...
    async execute(args, exec) {
      ...
      return submitJob(args.name, args.command, args.workdir, createdBySession, 'bat', mode)
    },
  })

  const disposeStatus = ctx.tools.register({
    name: 'bgjob_status',
    description: '查询一个 bgjob_submit 提交的后台任务状态：running/done、exitCode、最近输出尾部与日志路径。DSH 重启后旧任务 id 仍可查询（自动从磁盘恢复），任务不因重启而丢失。',
    presentCall: (args) => ({
      card: 'generic',
      title: '查询后台任务',
  ...
  const disposePwshSubmit = ctx.tools.register({
    ...
    description: '把 PowerShell 命令提交为独立于 DSH 进程的后台任务（与 bgjob_submit 同机制：schtasks 托管、网页面板实时输出、退出 Toast）。command 为 PowerShell 语法，由 PowerShell 执行（pwsh 7 优先、Windows PowerShell 5.1 兜底），输出日志 UTF-8（无 cmd GBK/UTF-8 乱码问题）；exit <code> 语义安全（不像 cmd 的 exit 提前终止导致 exitcode 不写入）；循环用 foreach / 1..5 | % {} 等 PowerShell 语法。workdir 必须是 DSH 工作区内的绝对路径。',
    presentCall: (args) => ({
      card: 'generic',
    ...
    async execute(args, exec) {
      ...
      return submitJob(args.name, args.command, args.workdir, createdBySession, 'pwsh', mode)
    },
  })
  ...
  return () => {
    ...
    for (const d of [disposeSubmit, disposeStatus, disposePwshSubmit]) { try { d() } catch (e) { /* noop */ } }
