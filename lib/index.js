...
/**
 * 生成 pwsh 引擎的任务 bat（run.bat）：直接以 PowerShell 解释器执行 job.ps1，
 * 输出整体重定向到日志。与 buildBat 结构一致：interpreter 是提交时解析出的
 * 绝对路径（防 schtasks 运行上下文 PATH 不一致）；保留 chcp 65001 兜底
 * （对 cmd 宿主自身输出无害）；末尾写 exitcode 并自删任务计划。
 */
export function buildPwshBat(job) {
  const scriptPath = job.meta.scriptPath || path.join(path.dirname(job.meta.jsonPath), 'job.ps1')
  const lines = [
    '@echo off',
    '>nul chcp 65001',
    'cd /d "' + job.meta.workdir + '"',
    '"' + job.meta.interpreter + '" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "' + scriptPath + '" >> "' + job.meta.logPath + '" 2>&1',
    'set "bgrc=%errorlevel%"',
    '>> "' + job.meta.logPath + '" echo [BGJOB] exit code: %bgrc%',
    '> "' + job.meta.exitcodePath + '" echo %bgrc%',
    'schtasks /Delete /TN ' + job.meta.taskName + ' /F >nul 2>&1',
...
export function apply(ctx) {
  ...
  const submitJob = async (jobName, command, workdirRaw, createdBySession, engine = 'bat') => {
    ...
    try {
      if (isPwsh) {
        // job.ps1 必须以 UTF-8 with BOM 写入：PowerShell 5.1 解析无 BOM 文件按 ANSI/GBK 读，中文会乱。
        const bom = Buffer.from([0xef, 0xbb, 0xbf])
        await fsp.writeFile(meta.scriptPath, Buffer.concat([bom, Buffer.from(buildPs1(job), 'utf8')]))
        await fsp.writeFile(batPath, buildPwshBat(job), 'utf8')
      } else {
  ...
  const disposePwshSubmit = ctx.tools.register({
    ...
    async execute(args, exec) {
      const createdBySession = exec && exec.agent && exec.agent.session ? String(exec.agent.session.id) : ''
      return submitJob(args.name, args.command, args.workdir, createdBySession, 'pwsh')
    },
  })
