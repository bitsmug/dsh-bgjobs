...
export function buildBgjobsGuidance() {
  return [
    ...
    '- bgjob_wait: 等待后台任务结束（默认最多 120 秒，可传 timeoutSeconds，范围 1–600），结束后立即返回退出码与日志尾部；需要等结果继续时用它，避免用前台 sleep 反复轮询。长任务仍推荐 bgjob_submit 的 notify。三种用法：① jobId 单个任务；② jobIds 数组任一先结束即返回（any 竞速，result+pending）；③ 全缺省=等当前会话提交的任务任一结束。',
    '- bgjob_wait_all: 传 jobIds 数组（或全缺省=本会话任务）等全部结束，返回 results（每任务退出码/日志尾）+ allDone；超时 timedOut:true 可续等。一次并行提交多个任务后，用 wait_all 收口比逐个 bgjob_wait 更省。',
    '- bgjob_list: 列出当前会话提交的全部任务（id/status/exitCode），配合 bgjob_wait / bgjob_wait_all 缺省使用。',
    '- 可选 wait：bgjob_submit / bgjob_submit_pwsh 传 `wait: <秒>`（1–600）会在提交成功后原地等待——语义同 bgjob_wait「全缺省」：有本会话任务时等任一先结束（any），会话不可识别时回退为等刚提交的这一个；超时返回 timedOut 快照，可再调 bgjob_wait 续等。',
...
export function apply(ctx) {
  ...
  const entryOf = (id, snap, seen, started) => {
    if (!snap) {
      return seen.has(id)
        ? { ok: true, jobId: id, status: 'removed', exitCode: null, name: '', tail: '', error: undefined }
        : { ok: false, jobId: id, error: 'job not found: ' + id }
    }
    seen.add(id)
  ...
  // any：任一 id 进入终态（done/removed/not found）即返回该结果。
  const waitAnyOf = async (ids0, timeoutMs) => {
    ...
    if (first) return { ok: true, anyDone: true, timedOut: false, waitedMs: 0, result: first, pending: ids.filter((x) => x !== first.jobId) }
    while (Date.now() - started < timeoutMs) {
      const age = Date.now() - started
      await sleepMs(Math.min(WAIT_POLL_MAX_MS, Math.max(WAIT_POLL_MS, age * WAIT_POLL_RATIO)))
      const hit = await findFirst()
      if (hit) return { ok: true, anyDone: true, timedOut: false, waitedMs: Date.now() - started, result: hit, pending: ids.filter((x) => x !== hit.jobId) }
    }
    // 超时：返回全部当前状态。
    ...
    const results = ids.map((id) => {
      ...
      return t || { ok: true, jobId: id, status: s.snap ? s.snap.status : 'removed', exitCode: s.snap && s.snap.exitCode != null ? s.snap.exitCode : null, name: s.snap ? s.snap.name : '', tail: s.snap ? (s.snap.tail || '') : '', timedOut: true, waitedMs: timeoutMs }
    })
    return { ok: true, anyDone: false, timedOut: true, waitedMs: timeoutMs, results }
  }
  // all：全部 id 进入终态才返回；超时返回逐 id 当前状态。
  const waitAllOf = async (ids0, timeoutMs) => {
    ...
    const summarize = () => ({
      ok: true, allDone: done.size === ids.length, timedOut: false,
      waitedMs: Date.now() - started,
      results: ids.map((id) => done.get(id) || { ok: true, jobId: id, status: 'running', exitCode: null, name: '', tail: '', timedOut: true, waitedMs: Date.now() - started }),
    })
    await markRound()
    if (done.size === ids.length) return summarize()
    while (Date.now() - started < timeoutMs) {
      const age = Date.now() - started
      await sleepMs(Math.min(WAIT_POLL_MAX_MS, Math.max(WAIT_POLL_MS, age * WAIT_POLL_RATIO)))
      await markRound()
    ...
    return { ok: true, allDone: false, timedOut: true, waitedMs: timeoutMs, results: ids.map((id) => done.get(id) || { ok: true, jobId: id, status: 'running', exitCode: null, name: '', tail: '', timedOut: true, waitedMs: timeoutMs }) }
  }
  // submit 的 wait：等价「bgjob_wait 全缺省」= 等本会话任一任务先结束（any）；
  // 会话不可解析或本会话无任务时回退为等刚提交的这一个（原行为兜底）。
  const waitSubmittedAny = async (jobId, exec, timeoutMs) => {
    ...
    if (sessionId) {
      const ids = sessionJobIds(sessionId)
  ...
  const disposeSubmit = ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        wait: { type: 'number', default: 0, description: '可选：提交成功后原地等待任务结束的秒数。0/缺省=不等待立即返回；>0 则提交后自动等待（范围 1–600，超出 clamp），结束立即返回退出码与日志尾；超时返回 timedOut:true 快照，可再调 bgjob_wait 续等。' },
  ...
  const disposeWaitAll = ctx.tools.register({
    name: 'bgjob_wait_all',
    description: '等待一批后台任务全部结束并返回每个任务的退出码/日志尾（默认最多 120 秒，可传 timeoutSeconds，范围 1–600）。jobIds 传任务 id 数组；缺省 = 等当前 agent 会话提交的任务全部结束。任一任务被删除(removed)或找不到(not found)也视为已结束计入 results；超时返回各任务当前状态 + allDone:false，可再次调用续等。',
    presentCall: (args) => ({
      card: 'generic',
      title: '等待全部后台任务',
      kind: 'execute',
    ...
    async execute(args, exec) {
      ...
      if (typeof args.jobId === 'string' && args.jobId) ids = [args.jobId]
        ...
        const sessionId = exec && exec.agent && exec.agent.session ? String(exec.agent.session.id) : ''
        if (sessionId) ids = sessionJobIds(sessionId)
      }
  ...
  const disposeList = ctx.tools.register({
    name: 'bgjob_list',
    description: '列出当前 agent 会话提交的全部后台任务（running 与 done 均含），返回 jobId/name/status/exitCode/finishedAt/workdir/logPath，供 bgjob_wait / bgjob_wait_all 缺省使用（同源：本会话 = 提交时所在 agent 会话）。',
  ...
  return () => {
    ...
    for (const d of [disposeSubmit, disposeStatus, disposeWait, disposeWaitAll, disposeList, disposePwshSubmit]) { try { d() } catch (e) { /* noop */ } }
