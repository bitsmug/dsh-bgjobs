...
export function buildBgjobsGuidance() {
  return [
    ...
    '- bgjob_wait: 等待后台任务结果（默认最多 120 秒，可传 timeoutSeconds，范围 1–600），结束后立即返回退出码与日志尾部。三种用法：① jobId 单个任务；② jobIds 数组任一先结束即返回（any 竞速，result+pending）；③ 全缺省=等「notify 视图」任一结束——即本会话中结果尚未交付（notify 完成通知未投递成功、也未被 wait 返回）的任务。wait 返回某任务结果即视为把结果交付给本会话（返回 notified:true，notify 视图剔除该任务，避免重复等同一结果）。',
...
export function apply(ctx) {
  ...
  return () => {
    ...
    for (const d of [disposeSubmit, disposeStatus, disposeWait, disposeWaitAll, disposeList, disposePending, disposePwshSubmit]) { try { d() } catch (e) { /* noop */ } }
