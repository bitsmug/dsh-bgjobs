...
export function apply(ctx) {
  ...
  const disposeSubmit = ctx.tools.register({
    name: 'bgjob_submit',
    description: '把命令提交为独立于 DSH 进程的后台任务：用户命令原样写入子 bat，经 schtasks 一次性任务交由任务计划程序服务托管（日志 UTF-8、末尾写 exitcode、自删任务计划），关 DSH/关终端不影响。网页「后台任务监控」面板实时显示输出，任务退出时弹 Toast；可选 notify/notify_mode 控制是否把完成通知同时送进创建它的 agent 会话（缺省不注入会话）。command 支持多行与 for/if 块结构（bat 语法，循环变量用 %%i），原样写入子 bat 执行；workdir 必须是 DSH 工作区内的绝对路径（任务文件与日志写在 <workdir>/.dsh/bgjobs/<id>/ 下）。本工具（bat 引擎）任务恒为全权限、无法沙箱化：当前会话被 dsh 沙箱策略受限时，仅当 bgjobs 面板 full access 开关开启才可用，否则提交被拒绝——需要受限后台任务请用 bgjob_submit_pwsh 的 sandbox 参数。',
    presentCall: (args) => ({
      card: 'generic',
      title: '提交后台任务',
      kind: 'execute',
