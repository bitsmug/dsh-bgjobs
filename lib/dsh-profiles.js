 ...
 * 两种写法都要吃掉：带引号的 JS 字符串（`!!js '"Bearer " + process.env.X'`）与**无引号表达式**
 * （`KEY: !!js process.env.X`）。只匹配带引号形式会让无引号写法落到 yaml 的未知 tag 分支——
 * 值退化成**普通字符串字面量**（被当配置静默导入，且控制台刷 TAG_RESOLVE_FAILED 警告），
 * 实测 r3/res/res2 的 `METASO_API_KEY: !!js process.env.METASO_API_KEY` 正是此坑。
 * `[^\n]*` 吃到行尾（含尾随注释——本函数只提取配置值，注释丢失无影响）；flow 风格
 * （如 `{ a: !!js x, b: 1 }`）会把同行后续内容一并吃掉，YAML.parse 通常直接失败并返回
 * parseError（UI 可见的显式错误，不是静默错导），属已知限制。
 */
function parsePatchText(text) {
  const hasJsTag = /!!js\b/.test(text)
  const prepared = hasJsTag ? text.replace(/!!js\b[^\n]*/g, "'" + JS_TAG_PLACEHOLDER + "'") : text
  try {
...
export async function readMcpConfigs(scope, options = {}) {
  ...
  let servers = extractMcpServers(parsed.data)
  // 安全网：文件里确有 !!js，却没有任何一条被逐条定位到（写在非常规位置/未知形态）→ 整批标红，
  // 宁可让用户手工核对，也不要把 JS 表达式当普通字符串字面量静默导入。
  if (hasJsTag && servers.length > 0 && !servers.some((s) => s.needsAttention === true)) {
    servers = servers.map((s) => ({
      ...s,
      needsAttention: true,
      attentionReason: '该文件含 !!js 表达式但未能逐条定位（非常规写法）；导入前请人工核对 env/headers',
    }))
  }
  return { scope: label, path: file, exists: true, servers, hasJsTag }
}
