// bgjobs client — DSH 设置面板「后台任务」section（v0.1.66）
...
//   3) 「打开离线 GUI」动作行 —— host POST /bgjobs/gui?action=open；
//   4) 「打开所在文件夹」动作行 —— host POST /bgjobs/gui?action=reveal（v0.1.68 起 explorer.exe 直开 tools 目录）；
//   5) GUI 脚本路径只读展示。
...
function SettingsSection({ t, guiPrefs, monitor }) {
  ...
  // 打开所在文件夹：host POST /bgjobs/gui?action=reveal（explorer.exe 直开 tools 目录）
  const doReveal = async () => {
    if (busy) return
    setBusy(true)
    setMessage(null)
    const res = await callHost('reveal')
