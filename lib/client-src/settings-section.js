...
//      path:<tools 目录>}，DSH 已验证的打开机制），端点不可用则回退 /bgjobs/gui?action=reveal；
//   5) GUI 脚本路径只读展示。
...
function SettingsSection({ t, guiPrefs, monitor }) {
  ...
  // 打开所在文件夹：优先第一方 open-in-app（DSH 已验证的 explorer 打开机制，仅收目录）
  const doReveal = async () => {
    ...
    if (!res.ok) {
      // 第一方端点未挂载（非 web profile 等）→ 回退自身 reveal（explorer 单参数）
