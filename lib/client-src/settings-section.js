// bgjobs client — DSH 设置面板「后台任务」section（v0.1.69；v0.1.85 加「默认等待超时」）
...
//   3) 「默认等待超时」数值项（guiPrefs，bgjob_wait 的 timeoutSeconds 缺省值；输入框 + 保存）；
//   4) 「打开离线 GUI」动作行 —— host POST /bgjobs/gui?action=open（schtasks 拉起，脱离宿主 job）；
//   5) 「打开所在文件夹」动作行 —— 优先第一方 POST /open-in-app/open（{app:'explorer',
...
const { useGuiPrefsEnabled, useGuiPrefsDisplay, useGuiPrefsElements, useGuiPrefsWaitTimeout } = require('./gui-prefs.js')
const { useMonitorVisible } = require('./monitor.js')

// 「字段显示」可配置字段（与 host store.js 的 DISPLAY_FIELDS 顺序一致）。
const DISPLAY_FIELD_KEYS = ['id', 'name', 'status', 'runtime', 'exitCode', 'workdir', 'command', 'createdAt', 'finishedAt']
...
function SettingsSection({ t, guiPrefs, monitor }) {
  ...
  // 默认等待超时（bgjob_wait 的 timeoutSeconds 缺省值；null = 不限时）：订阅生效值 + 输入框本地态。
  const waitTimeout = useGuiPrefsWaitTimeout(guiPrefs)
  const [waitInput, setWaitInput] = React.useState('')
  const [waitMsg, setWaitMsg] = React.useState(null) // { ok:boolean, text } | null
  ...
  // 生效值变化（首次回读 / 保存后宿主回显 / 别处改动）→ 回填输入框；空 = 不限时。
  React.useEffect(() => {
    setWaitInput(waitTimeout === null || waitTimeout === undefined ? '' : String(waitTimeout))
  }, [waitTimeout])
  ...
  // 「默认等待超时」数字输入框样式（88px，右对齐）。
  const numInput = {
    width: 88, padding: '5px 8px', borderRadius: 6, fontSize: 12, textAlign: 'right',
    background: 'transparent', color: 'var(--dsw-alias-label-primary)', border: '1px solid var(--dsw-alias-border-l2)',
  }
  ...
  // 保存「默认等待超时」：留空/0 = 不限时；非数/负数 → 就地报错、不发请求。
  const doSaveWaitTimeout = () => {
    const raw = String(waitInput === null || waitInput === undefined ? '' : waitInput).trim()
  ...
  return h('div', { style: { padding: '4px 0 8px' } },
    ...
    h('div', { style: row },
      ...
      h('div', { style: { flex: 'none', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 } },
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 6 } },
          h('input', {
            type: 'number', min: 0, step: 1, value: waitInput,
            placeholder: t('settings.timeout.placeholder'), title: t('settings.timeout.title'),
            onChange: (e) => { setWaitInput(e.target.value); setWaitMsg(null) },
            onKeyDown: (e) => { if (e.key === 'Enter') { e.preventDefault(); doSaveWaitTimeout() } },
            style: numInput,
          }),
