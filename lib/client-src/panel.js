...
const { stateColor, stateLabel, normalizePath, readActiveCwd, DONE_AGE_MS, PANEL_Z, TOAST_Z, GRIP_H, ToastComponent, iconEl, IconTrash, IconClock, IconClose, IconChevronDown, IconChevronRight, IconFolderOpen, IconFolderClose, IconSettings, IconDataOutline16, SelfToast, CtrlBtn, MiniBtn, Toggle, portalBody, clampToViewport } = require('./ui.js')

// 「字段显示」可配置字段（与 host store.js 的 DISPLAY_FIELDS 保持一致）与纯工具。
const FIELD_KEYS = ['id', 'name', 'status', 'runtime', 'exitCode', 'workdir', 'command', 'createdAt', 'finishedAt']
...
// 时长（毫秒 → 紧凑文本）：12s / 3m05s / 2h03m / 1d04h；非数/负值 → '-'。
...
const oneLine = (s) => String(s === null || s === undefined ? '' : s).replace(/\r?\n/g, ' ').trim()
    ...
    function Panel({ sessions: sessionsProp, getSessions, t, monitor, guiPrefs }) {
      ...
      // 运行时长：running → 「运行时长 X」（随每秒轮询增长）；done → 总耗时；时间戳缺失 → '-'。
      // 只出纯时长——列表次行本身会渲染成「字段名 值」，再带"已运行/耗时"会重复。
      ...
      const fieldText = (job, key) => {
        switch (key) {
          ...
          case 'runtime': return runtimeText(job)
          ...
          open
            ...
            ? h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 8px 6px 12px', borderBottom: '1px solid var(--dsw-alias-border-l1)', cursor: 'grab', touchAction: 'none', userSelect: 'none' }, onPointerDown: startDrag, onPointerMove: moveDrag, onPointerUp: endDrag, onPointerCancel: endDrag, title: t('drag.move') },
                ...
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 2 } },
                  ...
                  el('mcpSettingsButton') ? h(CtrlBtn, { title: t('gearbtn.mcpTitle'), onClick: () => {
                    ...
                    iconEl(IconDataOutline16, '🗄', 14)) : null,
