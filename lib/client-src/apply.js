...
const { McpSettings } = require('./mcp-section.js')
...
const { createGuiPrefsStore, loadGuiPrefs } = require('./gui-prefs.js')
    ...
    function apply(ctx) {
      ...
      // UI 偏好 store（左侧入口显隐 + 任务字段显示位置），设置页与面板共享同一实例。
      ...
      // DSH 设置面板：两个页面（settings.section 是 list seat，可注册多项）——
      // 「后台任务」（入口/面板/离线 GUI/字段显示）与「MCP 任务」（总开关 / server 登记 / 导入导出）
      // 拆开，避免单页过长。seat 由 ui-settings-general 的 sidebar.settings occupant 声明；
      // 无设置面板的组合静默等待 → 不回归。
      const bindNavT = () => {
