...
//   2) MCP 服务器登记区：列表（transport / 目标 / 预热状态 / 工具数）+ 每行「预热 / 冷启动 / 禁用」
//      三态按钮（琥珀/绿/灰，带文字）、「编辑」（载入明细，含 env/headers 值）、「列出工具」
//      （展开工具名，点击可复制）、「删除」，以及新增/覆盖表单（名字 + JSON 配置）；
...
function McpSettings({ t }) {
  ...
  // 三态切换：预热 = 启用 + 常驻连接；冷启动 = 启用但不常驻；禁用 = 拒绝 agent 调用并断开常驻连接。
  const doSetMode = (name, mode) => act(async () => {
    const q = mode === 'prewarm' ? '&enabled=1&prewarm=1'
      : (mode === 'cold' ? '&enabled=1&prewarm=0' : '&enabled=0')
  ...
  // 三态按钮：激活态用语义色描边+文字，非激活态置灰；靠文字+颜色双重区分（不用 Switch，避免与总开关混淆）。
  const modeBtn = (active, tone, key, label, title, fn) => h('div', {
    key, role: 'button', tabIndex: 0, title, 'aria-pressed': active ? 'true' : 'false',
    style: Object.assign({}, actBtn, {
      ...
      fontWeight: active ? 600 : 400, opacity: active ? 1 : 0.6,
    }),
    onClick: fn, onKeyDown: keyAct(fn),
  }, label)
  const MODE_TONE = {
