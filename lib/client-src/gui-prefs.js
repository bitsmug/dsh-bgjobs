...
// bgjob_wait 的 timeoutSeconds 缺省值（秒）；null = 不限时（与 host DEFAULT_WAIT_TIMEOUT_SECONDS 一致）。
...
function createGuiPrefsStore(initialEnabled = false) {
  ...
  // 默认等待超时（秒；null = 不限时）：本地即时生效 + 异步落盘（POST 里 0 表示不限时）。
  const applyWaitTimeout = (v) => {
    const next = normalizeWaitTimeout(v)
    if (next !== waitTimeoutSeconds) { waitTimeoutSeconds = next; emit() }
  }
...
// React hook：订阅「默认等待超时」（秒；null = 不限时）。store 缺位时返回 null。
...
module.exports = { createGuiPrefsStore, loadGuiPrefs, useGuiPrefsEnabled, useGuiPrefsDisplay, useGuiPrefsElements, useGuiPrefsWaitTimeout, DEFAULT_DISPLAY, DEFAULT_ELEMENTS }
