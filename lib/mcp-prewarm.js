...
// 定位：**只做加速**。任务 runner 先试代理（省 spawn + initialize）；**只有"确定没执行"的失败**
// 才回退冷启动——`/call` 响应带 `attempted: true` 表示"请求已发给 server（可能已执行）"，此时
// runner 不得重跑（防重复副作用），直接按退出码收尾。
...
import { openClient, listTools as listToolsOf, projectResult, clampTimerMs } from './mcp-connect.js'
...
export function createMcpPrewarm(deps) {
  ...
  async function call(server, tool, args, timeoutMs) {
    return withQueue(server, async () => {
      ...
      rec.lastUsedAt = Date.now()
      // timeoutMs 缺省/非法 → 不设调用超时（不限时）；有值则过 clampTimerMs（setTimeout 上限）。
      const t = Number(timeoutMs)
      const reqTimeout = Number.isFinite(t) && t > 0 ? clampTimerMs(t) : undefined
      try {
        const raw = await rec.client.callTool(
          { name: String(tool), arguments: args && typeof args === 'object' ? args : {} },
          undefined,
          reqTimeout !== undefined ? { timeout: reqTimeout } : undefined,
        )
        rec.lastUsedAt = Date.now()
        ...
        rec.lastUsedAt = Date.now()
        // 标记"请求已发出"：调用可能已在 server 侧执行 → 调用方（runner）不得回退冷启动重跑。
        // 必须保留原 message（超时判定与日志都读它）。
        const wrapped = new Error(errMsg(e))
        wrapped.attempted = true
        throw wrapped
      }
    })
  }
  ...
  function startProxy() {
    httpServer = http.createServer(async (req, res) => {
      // 客户端（runner）可能在中止预算到点后断连，之后我们再写响应会报错 → 兜住 error，写前再检查可写。
      res.on('error', () => { /* 客户端已断开，忽略 */ })
      ...
      try {
        ...
        // attempted=true：调用已经发给 server（可能已执行）→ 调用方不得重跑，只按退出码收尾。
        respond(200, { ok: false, error: errMsg(e), attempted: e && e.attempted === true })
      }
    })
