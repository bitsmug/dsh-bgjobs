    ...
    function Panel({ sessions }) {
      ...
      return portalBody(h(React.Fragment, null,
        ...
        h('div', { ref: panelRef, style: panelStyle },
          open
            // ── 标题栏（展开态）：标题 + dsh 图标按钮 ──
            ? h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 8px 6px 12px', borderBottom: '1px solid var(--dsw-alias-border-l1)', cursor: 'grab', touchAction: 'none', userSelect: 'none' }, onPointerDown: startDrag, onPointerMove: moveDrag, onPointerUp: endDrag, onPointerCancel: endDrag, title: '拖拽移动面板' },
                h('div', { style: { fontWeight: 700, fontSize: 13 } }, '后台任务监控' + (jobs.length ? ' (' + jobs.length + ')' : '')),
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 2 } },
                  h(CtrlBtn, { title: '清理已完成任务：点击选择「仅超过 24h」或「全部」（与视图过滤一致）', active: cleanupMenu, onClick: () => setCleanupMenu(!cleanupMenu) },
                    iconEl(IconTrash, '🧹', 14)),
                  h(CtrlBtn, { title: '折叠为仅任务列表（隐藏标题栏/筛选/分组路径，点击列表展开）', onClick: (e) => {
                  ...
                  h(CtrlBtn, { title: '最小化为悬浮球（球会出现在按钮位置）', onClick: (e) => minimizeToBall(e.currentTarget) },
                    iconEl(IconClose, '×', 16))
                )
              )
            ...
            : visibleJobs.length === 0
              ? h('div', { title: '点击展开面板', onPointerDown: startDrag, onPointerMove: moveDrag, onPointerUp: endDrag, onPointerCancel: endDrag, onClick: () => { if (!draggedRef.current) setOpen(true) }, style: { flex: 1, minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'grab', touchAction: 'none', userSelect: 'none' } },
              ...
              : h('div', { onPointerDown: startDrag, onPointerMove: moveDrag, onPointerUp: endDrag, onPointerCancel: endDrag, onClick: () => { if (!draggedRef.current) setOpen(true) }, style: { flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 0', cursor: 'grab', touchAction: 'none', userSelect: 'none' } },
                  visibleJobs.map((job) => h('div', { key: 'f-' + job.id, title: '点击展开完整面板', style: { display: 'flex', alignItems: 'center', gap: 8, height: 30, padding: '0 28px 0 10px', borderBottom: '1px solid var(--dsw-alias-border-l1)' } },
                    ...
                    h('span', { style: { color: stateColor(job), flex: 'none', fontSize: 11, fontWeight: 600 } }, stateLabel(job))
                  )),
                  // 右侧显式展开入口（无标题栏时唯一常显箭头）。
                  h('div', { 'data-bgjobs-ctrl': true, title: '展开完整面板', onClick: (e) => { e.stopPropagation(); setOpen(true) }, style: { position: 'absolute', right: 2, top: 2, width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 6, cursor: 'pointer', color: 'var(--dsw-alias-label-secondary)' } },
                    iconEl(IconChevronDown, '▸', 14))
                ),
          cleanupMenu && open ? h('div', { style: { borderBottom: '1px solid var(--dsw-alias-border-l1)', padding: '6px 10px 4px', display: 'flex', flexDirection: 'column', gap: 2, fontSize: 11 } },
            ...
            h('div', { onClick: () => { setCleanupMenu(false); cleanupVisible(true) }, title: '仅删除完成超过 24h 的任务（24h 内默认保留）', style: { padding: '3px 4px', cursor: 'pointer', borderRadius: 4, display: 'flex', alignItems: 'center', gap: 6 } },
              iconEl(IconClock, '🧹', 12), '仅清理超过 24h 的已完成任务'),
            h('div', { onClick: () => { setCleanupMenu(false); cleanupVisible(false) }, title: '删除当前视图中所有已完成任务（含 24h 内）', style: { padding: '3px 4px', cursor: 'pointer', borderRadius: 4, display: 'flex', alignItems: 'center', gap: 6 } },
              iconEl(IconTrash, '🗑', 12), '清理全部已完成任务'),
          ...
          open ? h('div', { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' } },
            ...
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, padding: '4px 12px 6px', fontSize: 11, opacity: 0.85 } },
              h(Toggle, { checked: onlyActive, onChange: setOnlyActive, title: '只显示当前 active 会话工作区（含其子目录）下的任务', label: '仅当前会话' }),
              ...
              h('div', { style: { marginLeft: 'auto', display: 'flex', alignItems: 'center' } },
                h(Toggle, { checked: fullAccess, onChange: toggleFullAccess, title: '预批准全权限后台任务（原模式）。开启后受限会话里的宽权限请求不再逐次弹审批；未挂载 dsh 沙箱策略服务的部署也必须开启才能提交任务。', label: '全权限', onColor: 'var(--dsw-alias-state-warn-primary)' })
              )
            ),
            // 列表区：可滚动；按工作区分组，组头吸顶、可点击折叠。
            h('div', { style: { flex: 1, minHeight: 0, overflowY: 'auto', padding: '0 8px 4px' } },
              visibleJobs.length === 0 ? h('div', { style: { opacity: 0.6, padding: 8 } }, (jobs.length === 0 ? '暂无后台任务（agent 用 bgjob_submit 提交）' : '当前会话工作区暂无任务（关闭筛选可查看全部）'))
              : h('div', null,
                groups.map((g) => h('div', { key: 'g-' + g.wd },
                  h('div', { onClick: () => toggleGroup(g.wd), title: collapsed.has(g.wd) ? '展开该工作区的任务' : '折叠该工作区的任务', style: { position: 'sticky', top: 0, zIndex: 1, background: 'var(--dsw-alias-bg-layer-3)', padding: '4px 8px', fontSize: 11, fontWeight: 600, opacity: 0.65, borderBottom: '1px solid var(--dsw-alias-border-l1)', cursor: 'pointer', userSelect: 'none', display: 'flex', alignItems: 'center' } },
