  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      const cleanupAll = async () => {
        const r = await callHost('/bgjobs/cleanup')
        if (r && r.ok) pollNow()
        return r
      }
      ...
      const panelH = size ? size.h : Math.min(460, Math.max(190, 120 + jobs.length * 44))
        ...
        h('div', { ref: panelRef, style: panelStyle },
          h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderBottom: '1px solid var(--dsw-alias-border-l1)', cursor: 'grab', touchAction: 'none', userSelect: 'none' }, onPointerDown: startDrag, onPointerMove: moveDrag, onPointerUp: endDrag, onPointerCancel: endDrag, title: '拖拽移动面板' },
            ...
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
              h('span', { 'data-bgjobs-ctrl': true, style: { opacity: 0.7, cursor: 'pointer', padding: '0 2px' }, onClick: (e) => { e.stopPropagation(); cleanupAll() }, title: '一键清理所有已完成任务' }, '🧹'),
          ...
          open ? h('div', { style: { display: 'flex', flexDirection: 'column', height: panelH - 40, overflow: 'hidden' } },
            // 列表区：可滚动；按工作区分组，组头吸顶。
            h('div', { style: { flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 8px' } },
              jobs.length === 0 ? h('div', { style: { opacity: 0.6, padding: 8 } }, '暂无后台任务（agent 用 bgjob_submit 提交）')
              : h('div', null,
                groups.map((g) => h('div', { key: 'g-' + g.wd },
                  h('div', { style: { position: 'sticky', top: 0, zIndex: 1, background: 'var(--dsw-alias-bg-layer-3)', padding: '4px 8px', fontSize: 11, fontWeight: 600, opacity: 0.65, borderBottom: '1px solid var(--dsw-alias-border-l1)' } }, '📁 ' + g.wd),
                  g.jobs.map((job) => h('div', { key: job.id,
