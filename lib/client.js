  ...
  "lib/client-src/settings-section.js"(exports2, module2) {
    ...
    function SettingsSection({ t, guiPrefs }) {
      ...
      const callGui = (action) => {
        ...
        setBusy(true);
        setMessage(null);
        fetch("/bgjobs/gui?action=" + action, { method: "POST" }).then((r) => r.json()).then((d) => {
          setMessage({ ok: !!(d && d.ok), text: d && d.ok ? t("settings.gui.opened") : t("settings.gui.failed", { error: String(d && d.error || "unknown") }) });
        }).catch((e) => {
          setMessage({ ok: false, text: t("settings.gui.failed", { error: String(e && e.message || e) }) });
      ...
      return h(
        ...
        h(
          ...
          SwitchC ? h(SwitchC, { checked: enabled, onChange: (v) => guiPrefs && guiPrefs.setEnabled(!!v), label: t("settings.entryLabel") }) : h(
            "label",
            { style: { display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", flex: "none", fontSize: 13, color: "var(--dsw-alias-label-primary)" } },
            h("input", { type: "checkbox", checked: enabled, onChange: (e) => guiPrefs && guiPrefs.setEnabled(e.target.checked) }),
            enabled ? t("footer.hide") : t("footer.show")
          )
        ),
        ...
        h(
          ...
          h(
            "div",
            {
              role: "button",
              tabIndex: 0,
              title: t("settings.gui.reveal"),
              style: Object.assign({}, actBtn, { background: "transparent" }),
