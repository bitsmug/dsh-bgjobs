                ...
                groups.map((g) => h('div', { key: 'g-' + g.wd },
                  ...
                  collapsed.has(g.wd) ? null : g.jobs.map((job) => h('div', { key: job.id,
                    ...
                    sel && sel.id === job.id ? h('div', { style: { padding: '4px 8px 8px' } },
                      ...
                      h('pre', { ref: logRef, style: { margin: 0, padding: 8, background: 'var(--dsw-specific-input-major)', borderRadius: 6, maxHeight: 280, overflowY: 'auto', fontSize: 12, lineHeight: 1.5, whiteSpace: 'pre-wrap', wordBreak: 'break-all', color: 'var(--dsw-alias-label-primary)' } }, (job.tail || (job.status === 'done' ? t('log.none') : t('log.waiting'))))
                    ) : null
                  ))
                ))
              )
