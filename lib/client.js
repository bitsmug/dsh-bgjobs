...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel({ sessions }) {
      ...
      // ── 完整面板 ────────────────────────────────────────────────────
      // 仅显示当前会话工作区（activeCwd 不可用时退化为全部）。
      const activeNorm = activeCwd ? normalizePath(activeCwd) : ''
      const visibleJobs = onlyActive && activeNorm ? jobs.filter((j) => normalizePath(j.workdir) === activeNorm) : jobs
      // 按工作区分组（保持任务原有相对顺序）。
      const groups = []
      ...
      return h(React.Fragment, null,
        ...
        h('div', { ref: panelRef, style: panelStyle },
          ...
          open ? h('div', { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' } },
            // 工具栏：仅当前会话工作区过滤开关（随 active 会话自动切换）。
