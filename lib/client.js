...
var require_i18n = __commonJS({
  "lib/client-src/i18n.js"(exports2, module2) {
    ...
    var ZH = {
      ...
      // 默认等待超时（bgjob_wait 的 timeoutSeconds 缺省值）
      "settings.timeout.title": "默认等待超时",
      "settings.timeout.desc": "agent 调用 bgjob_wait 未指定 timeoutSeconds 时使用；0 或留空 = 不限时。",
      "settings.timeout.placeholder": "不限时",
      "settings.timeout.save": "保存",
      "settings.timeout.saved": "已保存",
      "settings.timeout.failed": "保存失败：{error}",
    ...
    var EN = {
      ...
      // Default wait timeout (the default value of bgjob_wait's timeoutSeconds)
      "settings.timeout.title": "Default wait timeout",
      "settings.timeout.desc": "Used when the agent calls bgjob_wait without timeoutSeconds; 0 or empty = no limit.",
      "settings.timeout.placeholder": "unlimited",
      "settings.timeout.save": "Save",
      "settings.timeout.saved": "Saved",
      "settings.timeout.failed": "Save failed: {error}",
...
var require_gui_prefs = __commonJS({
  "lib/client-src/gui-prefs.js"(exports2, module2) {
    ...
    var DEFAULT_WAIT_TIMEOUT_SECONDS = null;
    var normalizeWaitTimeout = (v) => typeof v === "number" && Number.isFinite(v) && v > 0 ? Math.floor(v) : null;
    function createGuiPrefsStore(initialEnabled = false) {
      ...
      const applyWaitTimeout = (v) => {
        const next = normalizeWaitTimeout(v);
        if (next !== waitTimeoutSeconds) {
          waitTimeoutSeconds = next;
          emit();
        }
      };
      const setWaitTimeout = (sec) => {
        const next = normalizeWaitTimeout(Number(sec));
        applyWaitTimeout(next);
        fetch("/bgjobs/uiprefs", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ waitTimeoutSeconds: next === null ? 0 : next })
      ...
      const applyHostPrefs = (d) => {
        ...
        if ("waitTimeoutSeconds" in d) applyWaitTimeout(d.waitTimeoutSeconds);
      ...
      return {
        ...
        getWaitTimeout: () => waitTimeoutSeconds,
        applyWaitTimeout,
        setWaitTimeout,
        setDisplayField,
        setElement,
    ...
    function loadGuiPrefs(store) {
      fetch("/bgjobs/uiprefs").then((r) => r.json()).then((d) => {
        ...
        if ("waitTimeoutSeconds" in d) store.applyWaitTimeout(d.waitTimeoutSeconds);
    ...
    function useGuiPrefsWaitTimeout(store) {
      const subscribe = React.useCallback((cb) => store ? store.subscribe(cb) : () => {
      }, [store]);
      const getSnapshot = React.useCallback(() => store ? store.getWaitTimeout() : null, [store]);
      return React.useSyncExternalStore(subscribe, getSnapshot);
    }
    module2.exports = { createGuiPrefsStore, loadGuiPrefs, useGuiPrefsEnabled, useGuiPrefsDisplay, useGuiPrefsElements, useGuiPrefsWaitTimeout, DEFAULT_DISPLAY, DEFAULT_ELEMENTS };
  }
});
...
var require_panel = __commonJS({
  "lib/client-src/panel.js"(exports2, module2) {
    ...
    var FIELD_KEYS = ["id", "name", "status", "runtime", "exitCode", "workdir", "command", "createdAt", "finishedAt"];
    ...
    var fmtDuration = (ms) => {
      ...
      if (m < 60) return String(m) + "m" + pad2(s % 60) + "s";
      const hh = Math.floor(m / 60);
      if (hh < 24) return String(hh) + "h" + pad2(m % 60) + "m";
      return String(Math.floor(hh / 24)) + "d" + pad2(hh % 24) + "h";
    };
    ...
    function Panel({ sessions: sessionsProp, getSessions, t, monitor, guiPrefs }) {
      ...
      const runtimeText = (job) => {
        const start = Number(job.createdAt);
        if (!Number.isFinite(start) || start <= 0) return "-";
        const end = job.status === "done" ? Number(job.finishedAt) : Date.now();
...
var require_settings_section = __commonJS({
  "lib/client-src/settings-section.js"(exports2, module2) {
    ...
    var h = React.createElement;
    var { makeT } = require_i18n();
    var { useGuiPrefsEnabled, useGuiPrefsDisplay, useGuiPrefsElements, useGuiPrefsWaitTimeout } = require_gui_prefs();
    var { useMonitorVisible } = require_monitor();
    var DISPLAY_FIELD_KEYS = ["id", "name", "status", "runtime", "exitCode", "workdir", "command", "createdAt", "finishedAt"];
    ...
    function SettingsSection({ t, guiPrefs, monitor }) {
      ...
      const waitTimeout = useGuiPrefsWaitTimeout(guiPrefs);
      const [waitInput, setWaitInput] = React.useState("");
      const [waitMsg, setWaitMsg] = React.useState(null);
      ...
      React.useEffect(() => {
        setWaitInput(waitTimeout === null || waitTimeout === void 0 ? "" : String(waitTimeout));
      }, [waitTimeout]);
      ...
      const doSaveWaitTimeout = () => {
        const raw = String(waitInput === null || waitInput === void 0 ? "" : waitInput).trim();
        const sec = raw === "" ? 0 : Number(raw);
        if (!Number.isFinite(sec) || sec < 0) {
          setWaitMsg({ ok: false, text: t("settings.timeout.failed", { error: raw }) });
          return;
        }
        if (guiPrefs && typeof guiPrefs.setWaitTimeout === "function") guiPrefs.setWaitTimeout(sec);
        setWaitMsg({ ok: true, text: t("settings.timeout.saved") });
      };
      const fldRow = (f) => {
      ...
      return h(
        ...
        // 行 2.6：默认等待超时（bgjob_wait 的 timeoutSeconds 缺省值；留空/0 = 不限时）
        h(
          "div",
          { style: row },
          h(
            "div",
            { style: labelCol },
            h("div", { style: labelMain }, t("settings.timeout.title")),
            h("div", { style: labelSub }, t("settings.timeout.desc"))
          ),
          h(
            "div",
            { style: { flex: "none", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 } },
            h(
              "div",
              ...
              h("input", {
                ...
                step: 1,
                value: waitInput,
                placeholder: t("settings.timeout.placeholder"),
              ...
              h("div", { role: "button", tabIndex: 0, title: t("settings.timeout.save"), style: actBtn, onClick: doSaveWaitTimeout, onKeyDown: keyAct(doSaveWaitTimeout) }, t("settings.timeout.save"))
            ),
            waitMsg ? h("div", { role: "status", style: { fontSize: 12, color: waitMsg.ok ? "var(--dsw-alias-state-success-primary)" : "var(--dsw-alias-label-secondary)" } }, waitMsg.text) : null
          )
        ),
