...
import { mcpServerDisabledError } from './store.js'
...
export function createMcp(ctx, store, deps) {
   ...
   * 登记 server 被设为「禁用」时抛错，除非 options.allowDisabled === true（设置页「列出工具」
   * 用它放行，禁用后仍可手动探测）。
   * @throws 参数非法 / server 未登记 / server 已禁用 / 配置不合法（fail loud，由工具层转成 { ok:false, error }）。
   */
  const resolveServer = async (input, options) => {
    ...
    if (!enabled && !(options && options.allowDisabled === true)) throw new Error(mcpServerDisabledError(name))
    return {
      name,
      config: normalizeConfig(raw),
      prewarm: enabled && raw.prewarm === true,
