...
export function createRegistry(ctx, store) {
  ...
  const view = (job) => ({
    ...
    // 引擎标签（bat / pwsh / mcp）+ mcp 任务的执行通道（prewarm / cold，完成后由 watch 落盘）
    engine: job.meta.engine || 'bat',
    channel: job.meta.mcpChannel === undefined || job.meta.mcpChannel === null ? null : String(job.meta.mcpChannel),
    // 面板「字段显示」可配置项：command（任务命令）与 createdAt（开始时间）。
    command: job.meta.command === undefined || job.meta.command === null ? '' : String(job.meta.command),
    createdAt: job.meta.createdAt === undefined || job.meta.createdAt === null ? null : Number(job.meta.createdAt),
  ...
  const statusFromDisk = async (jobId) => {
    ...
    return {
      ...
      engine: meta.engine || 'bat',
      channel: meta.mcpChannel === undefined || meta.mcpChannel === null ? null : String(meta.mcpChannel),
