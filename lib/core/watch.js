...
export function createWatch(ctx, store, deps) {
  ...
  const checkCompletion = async (job) => {
    ...
    try {
      ...
      // mcp 任务额外记录执行通道（prewarm/cold，来自 runner 的 result.json）供面板/详情展示。
      if (job.meta.engine === 'mcp') {
        try {
          const rj = JSON.parse(await fsp.readFile(path.join(job.meta.jobDir || path.dirname(job.meta.jsonPath), 'result.json'), 'utf8'))
          if (rj && typeof rj.channel === 'string') job.meta.mcpChannel = rj.channel
        } catch (e) { /* 结果文件缺失（如任务被强杀）：不记通道 */ }
      }
