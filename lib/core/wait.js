...
// 故在轮询内对 exec.agent.inbox 做「wait 起点基准 + 差分」检测，出现任意新消息即返回
// stoppedBy:'message' 让路（signal abort → stoppedBy:'signal'）。inbox 不存在则不启用。
...
// 单一任务当前状态的「stopped」返回（用户打断/新消息让路时用；不置 delivered）。
...
export function createWait(ctx, store, deps) {
  ...
  // 用户停止/打断（signal abort）→ stoppedBy:'signal'；新入站消息（watchMsg）→ stoppedBy:'message'；
  // 均立即返回 stopped:true 快照（不置 delivered，可续等）。
  const waitJobDone = async (jobId, timeoutMs, signal, watchMsg) => {
    ...
    // 到这里 = 超时，或被打断/新消息让路（任务仍未结束；终态优先已在上面短路）。
    if (signal && signal.aborted) return stoppedSingle(jobId, s, Date.now() - started, 'signal')
    if (watchMsg && watchMsg()) return stoppedSingle(jobId, s, Date.now() - started, 'message')
  ...
  // signal abort（用户停止/打断）或新入站消息（watchMsg）→ 立即返回 stopped:true（不置 delivered）。
  const waitAnyOf = async (ids0, timeoutMs, signal, watchMsg) => {
    ...
    if (stoppedBy) return { ok: true, anyDone: false, timedOut: false, stopped: true, stoppedBy, waitedMs: Date.now() - started, results }
  ...
  const waitAllOf = async (ids0, timeoutMs, signal, watchMsg) => {
    const ids = uniqueIds(ids0)
    const started = Date.now()
    const seen = new Set()
  ...
  const waitSubmittedAny = async (jobId, exec, timeoutMs) => {
    ...
    return waitJobDone(jobId, timeoutMs, signal, watchMsg)
  }
