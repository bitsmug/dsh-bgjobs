...
// bgjob_wait（logic any|all；all 即原 bgjob_wait_all，v0.1.83 合并）/ 提交 wait 参数 / notify 视图（缺省等待集合）。
...
// 不必靠 agent 自觉收敛回合。另：bgjob_wait 的 logic:'all' 模式（原 bgjob_wait_all）为合取语义
// （全部成功才 allDone，任一失败即短路返回）。
...
export function createWait(ctx, store, deps) {
  ...
  // timeoutMs 可为 Infinity（缺省不限时，见 tools.js）；未知 id 返回 not found；等待中被清理 → status:'removed'；
  // 超时返回 timedOut 快照（仅在传了有限超时时可能发生）；
  ...
  const waitAllOf = async (ids0, timeoutMs, signal, watchMsg, conclude) => {
    ...
    const abortError = () => stoppedError('bgjob_wait', Date.now() - started, ids.map((id) => {
      const e = done.get(id)
