...
export function createWait(ctx, store, deps) {
  ...
  // 新入站消息（watchMsg）→ 成功返回 stopped:true / stoppedBy:'message' 快照。两者都不置 delivered、可续等。
  const waitJobDone = async (jobId, timeoutMs, signal, watchMsg) => {
    ...
    // 新入站消息让路：**成功返回** stopped 快照（不涉及 abort，值能正常送达）。
  ...
  // 用户停止/打断（signal abort）→ **抛结构化错误**（见 stoppedError）；新入站消息（watchMsg）→ 成功返回 stopped 快照。
  const waitAnyOf = async (ids0, timeoutMs, signal, watchMsg) => {
    ...
    if (stoppedBy === 'message') return { ok: true, anyDone: false, timedOut: false, stopped: true, stoppedBy, waitedMs: Date.now() - started, results }
  ...
  const waitAllOf = async (ids0, timeoutMs, signal, watchMsg) => {
    ...
    if (done.size === ids.length) {
      if (signal && signal.aborted) throw stoppedError('bgjob_wait_all', Date.now() - started, ids.map((id) => { const e = done.get(id); return { id, status: e ? e.status : 'unknown', exitCode: e ? e.exitCode : null } }))
    ...
    const stoppedBy = signal && signal.aborted ? 'signal' : (watchMsg && watchMsg() ? 'message' : null)
    if (stoppedBy === 'signal') {
      throw stoppedError('bgjob_wait_all', Date.now() - started, ids.map((id) => {
        const e = done.get(id)
