...
export function createStore() {
  ...
  // ── 网页 UI 偏好（DSH 设置页）：左侧显隐按钮开关。持久化
  //    $DSH_HOME/bgjobs/ui-prefs.json；缺省（无文件/旧版升级）为 false（默认隐藏，
  //    用户可在设置开启）。语义与 fullaccess（权限）无关，独立文件避免耦合。
  ...
  const readUiPrefs = async () => {
    ...
    try {
      ...
      value = { sidebarEntry: parsed.sidebarEntry === true }
    } catch (e) { /* 缺文件/坏文件 → 缺省 false */ }
    uiPrefsCache = value
    return uiPrefsCache
  }
  const setUiPrefs = async (patch) => {
    const current = { ...(uiPrefsCache !== null ? uiPrefsCache : { sidebarEntry: false }) }
    uiPrefsCache = { ...current, sidebarEntry: patch.sidebarEntry === true }
    try {
      const p = uiPrefsPath()
      await fsp.mkdir(path.dirname(p), { recursive: true })
      await fsp.writeFile(p, JSON.stringify(uiPrefsCache, null, 2), 'utf8')
    } catch (e) { /* 尽力而为 */ }
    return { ok: true, ...uiPrefsCache }
  }

  return { registry, readFullAccess, setFullAccess, readUiPrefs, setUiPrefs }
}
