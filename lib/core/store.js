...
export function createStore() {
  ...
  const readMcpPrefs = async () => {
    ...
    try {
      mcpPrefsCache = { enabled: JSON.parse(await fsp.readFile(mcpPrefsPath(), 'utf8')).enabled === true }
    } catch (e) { mcpPrefsCache = { enabled: false } }
  ...
  const normalizeServerConfig = (raw, prewarmFallback) => {
    ...
    if (raw && typeof raw === 'object') {
      ...
      else if (prewarmFallback !== undefined) out.prewarm = prewarmFallback === true
    }
    if (typeof out.prewarm !== 'boolean') out.prewarm = false
    return out
  }
  const readMcpServers = async () => {
    ...
    let servers = {}
    try {
      const parsed = JSON.parse(await fsp.readFile(mcpServersPath(), 'utf8'))
      if (parsed && parsed.servers && typeof parsed.servers === 'object') servers = parsed.servers
    } catch (e) { /* 缺文件/坏文件 → 空清单 */ }
  ...
  /** 新增/覆盖一个 server（同名覆盖；未显式给 prewarm 时保留原值）。 */
  const setMcpServer = async (name, config) => {
    ...
    let normalized
    try { normalized = normalizeServerConfig(config, existing ? existing.prewarm : false) } catch (e) {
  ...
  /** 切换某 server 的预热标记；server 未登记 → fail loud。 */
  const setMcpServerPrewarm = async (name, enabled) => {
    const key = String(name === undefined || name === null ? '' : name).trim()
    const current = await readMcpServers()
    const existing = current.servers[key]
    if (existing === undefined) return { ok: false, error: 'server not found: ' + key }
    mcpServersCache = { servers: { ...current.servers, [key]: { ...existing, prewarm: enabled === true } } }
    return await persistMcpServers()
  }
