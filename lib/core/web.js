...
import { readMcpConfigs, describeDshMcp } from '../dsh-profiles.js'
  ...
  const disposeRoutes = ctx.inject(['webServer'], (webCtx) => {
    const handler = async (req, res) => {
      ...
      if (pathname === '/bgjobs/mcpprefs') {
        if (process.env.BGJOBS_TRACE === '1') process.stderr.write('[trace] mcpprefs method=' + String(req.method) + ' raw=' + String(url.searchParams.get('enabled')) + '\n')
