...
export function createTools(ctx, store, deps) {
  ...
  const disposeWait = ctx.tools.register({
    name: 'bgjob_wait',
    description: '等待后台任务结束并立即返回结果（轮询，默认最多 120 秒，可传 timeoutSeconds 覆盖，范围 1–600）。三种用法：① 传单个 jobId——等该任务结束返回其退出码/日志尾（单任务）；② 传 jobIds 数组——任一先结束立即返回（any 竞速，结果含已完成者与仍在等列表），**并行提交多个任务时这是默认姿势**：先拿到先处理，其余用 pending 继续等，不必等齐；③ jobId/jobIds 都缺省——等当前 agent 会话提交的任务任一结束（本会话=bgjob_list 同源过滤）。单任务与多任务语义一致：done/removed/not found 均视为结束；超时返回 timedOut:true 快照，可再次调用。用户点击停止/打断会让本次等待**立即以错误结束**（错误文案含各任务当前状态与续等指引——这是 DSH 的取消语义：已启动的调用在调用方取消后不能被当作成功返回），任务继续后台运行、不标记已交付，可再次调用续等；等待期间收到新入站消息（其它 agent 的 send_message、用户 steer / 排队的新提示等）则**正常返回** stopped:true / stoppedBy 为 message，**并声明本次调用终结当前回合**——该返回不含消息正文，DSH 会随即把该消息作为正式用户消息投递给你的下一次生成（排队为本轮之后新提示的则由新回合投递）。收到后就先处理这条消息；任务继续后台跑、不置已交付，需要结果时再调用续等。不要用前台 sleep / 循环查状态代替本工具。',
    presentCall: (args) => ({
      card: 'generic',
      title: '等待后台任务',
      kind: 'execute',
  ...
  const disposeWaitAll = ctx.tools.register({
    name: 'bgjob_wait_all',
    description: '等待一批后台任务**全部成功**结束并返回每个任务的结果（默认最多 120 秒，可传 timeoutSeconds，范围 1–600）。jobIds 传任务 id 数组；缺省 = 等「notify 视图」（本会话尚未交付的任务）全部结束。合取语义：全部成功才返回 allDone:true + results（每任务含退出码/日志尾/notified）；**任一任务失败即立刻返回** `failed:true` + `failedJobId` + 已结束者的 results + 其余 pending（非 0 退出码，或被清理/找不到=无法确认成功，都算失败）——合取已确定为假，不再等剩余任务。因此它只用于「必须全部成功才能继续」或「一个失败就马上停」；只为等齐而用它会让最早完成的任务白等，那种场景请用 bgjob_wait 的 any 竞速。done 结果返回即置已交付（notified=true，notify 视图剔除）；removed/not found 也视为已结束计入 results；超时返回各任务当前状态 + allDone:false（未交付），可再次调用续等。用户点击停止/打断会让本次等待**立即以错误结束**（错误文案含各任务当前状态与续等指引——DSH 的取消语义：已启动调用在取消后不能被当作成功返回），任务继续后台运行、不标记已交付；等待期间收到新入站消息则**正常返回** stopped:true / stoppedBy 为 message，**并声明本次调用终结当前回合**——该返回不含消息正文，DSH 会随即把该消息作为正式用户消息送达；收到后先处理这条消息，任务照旧后台跑，之后再续等。不要用前台 sleep / 循环查状态代替本工具。',
    presentCall: (args) => ({
      card: 'generic',
      title: '等待全部后台任务',
      kind: 'execute',
