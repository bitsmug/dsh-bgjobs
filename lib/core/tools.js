  ...
  const disposeWait = ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        timeoutSeconds: { type: 'number', description: '可选：最多等待秒数。**缺省（不传）＝不限时**——一直等到任务结束（或被新入站消息让路 / 被用户停止）；传任意正数秒（含小数）才到点返回 timedOut:true 快照，可再调用续等' },
    ...
    async execute(args, exec) {
      const timeoutArg = Number(args.timeoutSeconds)
      // 缺省/非法值 → Infinity（不限时）；循环用墙钟判定，不会产生长定时器。
      const timeoutMs = Number.isFinite(timeoutArg) && timeoutArg > 0 ? timeoutArg * 1000 : Infinity
