...
export function createTools(ctx, store, deps) {
  ...
  const disposeWait = ctx.tools.register({
    name: 'bgjob_wait',
    description: '等待后台任务结束并立即返回结果（轮询，默认最多 120 秒，可传 timeoutSeconds 覆盖，范围 1–600）。三种用法：① 传单个 jobId——等该任务结束返回其退出码/日志尾（单任务）；② 传 jobIds 数组——任一先结束立即返回（any 竞速，结果含已完成者与仍在等列表）；③ jobId/jobIds 都缺省——等当前 agent 会话提交的任务任一结束（本会话=bgjob_list 同源过滤）。单任务与多任务语义一致：done/removed/not found 均视为结束；超时返回 timedOut:true 快照，可再次调用。用户点击停止/打断（stoppedBy 为 signal）、或等待期间收到新入站消息（其它 agent 的 send_message、用户 steer 等，stoppedBy 为 message）都会立即返回 stopped:true 快照（任务继续后台运行、不标记已交付，可再次调用续等）。',
    presentCall: (args) => ({
      card: 'generic',
      title: '等待后台任务',
      kind: 'execute',
  ...
  const disposeWaitAll = ctx.tools.register({
    name: 'bgjob_wait_all',
    description: '等待一批后台任务全部结束并返回每个任务的结果（默认最多 120 秒，可传 timeoutSeconds，范围 1–600）。jobIds 传任务 id 数组；缺省 = 等「notify 视图」（本会话尚未交付的任务）全部结束。返回 results（每任务含退出码/日志尾/notified）+ allDone；done 结果返回即置已交付（notified=true，notify 视图剔除）。removed/not found 也视为已结束计入 results；超时返回各任务当前状态 + allDone:false（未交付），可再次调用续等。用户点击停止/打断（stoppedBy 为 signal）、或等待期间收到新入站消息（其它 agent 的 send_message、用户 steer 等，stoppedBy 为 message）都会立即返回 stopped:true + 各任务当前状态（不标记已交付，可再次调用续等）。',
    presentCall: (args) => ({
      card: 'generic',
      title: '等待全部后台任务',
      kind: 'execute',
  ...
  const disposeMcpSubmit = ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        wait: { type: 'number', default: 0, description: '可选：提交成功后原地等待的秒数。0/缺省=不等待立即返回；>0 则内部按 bgjob_wait「全缺省」等待（有会话任务时等 notify 视图任一先交付即返回）。范围 1–600；超时返回 timedOut:true 快照可续等。' },
        notify: { type: 'string', enum: BGJOB_NOTIFY_MODES, default: 'off', description: '可选：任务结束后是否通知创建它的 agent 会话。off=不通知（缺省，仅网页 Toast）；on-completion=仅成功（exit 0）时通知；on-fail=仅异常退出（exit≠0）时通知；on-exit=任何退出都通知。' },
  ...
  const disposeMcpTools = ctx.tools.register({
    ...
    description: '列出某个 MCP server 的注册工具（工具名 + 描述 + 必填字段名）——提交 bgjob_submit_mcp 前先用它确认工具名与参数形状。server 与 server_config 二选一（同 bgjob_submit_mcp）；refresh=true 绕过 10 分钟缓存强制重取。优先用 DSH 已注册的 mcp__<server>__* 工具（零启动开销，不 spawn server），未命中才实际连接/启动 server 探测（该 server 预热开启时走常驻连接）；返回 source: registry|cache|probe 与 channel: prewarm|cold。**受「MCP 任务」开关限制**（关闭时拒绝；设置页 server 行的「列出工具」属用户显式操作，不受限）。',
    presentCall: (args) => ({
      card: 'generic',
      title: '列出 MCP 工具',
      kind: 'execute',
