...
export function createTools(ctx, store, deps) {
  ...
  const disposeWait = ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      if (typeof args.jobId === 'string' && args.jobId) return waitJobDone(args.jobId, timeoutMs, signal)
      // ② jobIds 数组 → any 竞速。
      if (Array.isArray(args.jobIds) && args.jobIds.length > 0) return waitAnyOf(args.jobIds, timeoutMs, signal)
  ...
  const disposeWaitAll = ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      return waitAllOf(ids, timeoutMs, signal)
    },
  })
