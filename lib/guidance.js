...
export function buildBgjobsGuidance() {
  return [
    ...
    '- bgjob_wait: 等待后台任务结果（默认最多 120 秒，可传 timeoutSeconds，范围 1–600），结束后立即返回退出码与日志尾部。三种用法：① jobId 单个任务；② jobIds 数组任一先结束即返回（any 竞速，result+pending）；③ 全缺省=等「notify 视图」任一结束——即本会话中结果尚未交付（notify 完成通知未投递成功、也未被 wait 返回）的任务。wait 返回某任务结果即视为把结果交付给本会话（返回 notified:true，notify 视图剔除该任务，避免重复等同一结果）。等待会被两件事打断：① 用户在等待中点停止/打断 → 本次等待**以错误立即结束**（错误文案含各任务当前状态与续等指引；这是 DSH 的取消语义——已启动的调用在取消后不能被当作成功返回），任务继续后台跑、不置已交付，收到该错误不要当失败，直接再次 bgjob_wait 续等即可；② 其它 agent 给你发来消息（send_message / 用户 steer 等）→ **正常返回** stopped:true（stoppedBy 为 message）快照，同样不置已交付、任务继续跑，请先处理这条消息，之后可再次 bgjob_wait 续等，不要重复提交任务。',
    ...
    '- 可选 wait：bgjob_submit / bgjob_submit_pwsh / bgjob_submit_mcp 传 `wait: <秒>`（1–600）会在提交成功后原地等待——语义同 bgjob_wait「全缺省」：有 notify 视图任务时等任一先交付（any），会话不可识别/视图为空时回退为等刚提交的这一个；超时返回 timedOut 快照，可再调 bgjob_wait 续等。',
    ...
    '长任务交给 bgjob_submit / bgjob_submit_pwsh / bgjob_submit_mcp 而不是前台执行；任务完成时网页顶部弹出 Toast 提示（不注入会话消息）。',
  ].join('\n')
}
