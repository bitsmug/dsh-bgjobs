...
export function buildBgjobsGuidance() {
  return [
    ...
    '- bgjob_wait: 等待后台任务结果（默认最多 120 秒，可传 timeoutSeconds，范围 1–600），结束后立即返回退出码与日志尾部。三种用法：① jobId 单个任务；② jobIds 数组任一先结束即返回（any 竞速，result+pending）；③ 全缺省=等「notify 视图」任一结束——即本会话中结果尚未交付（notify 完成通知未投递成功、也未被 wait 返回）的任务。wait 返回某任务结果即视为把结果交付给本会话（返回 notified:true，notify 视图剔除该任务，避免重复等同一结果）。用户在等待中点停止/打断时 wait 会立即返回 stopped:true（任务继续后台跑、结果未交付）——后续可再次 bgjob_wait 续等，不要因此重复提交任务。',
