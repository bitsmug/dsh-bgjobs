...
export function jobSandboxDecision(state, requested, engine, fullAccess) {
  // mcp 引擎恒 off 且**不受会话访问模式限制**（在 state 校验与 full access 判定之前早退）：
  // DSH 的 sandbox policy 只被实施隔离的执行通道消费（shell 沙箱执行器 ctx.sandbox.confine、
  // fs-sandbox、terminal-bash）；MCP server 由官方 SDK 的 StdioClientTransport 直接 spawn，
  // 不经 ctx.subprocess 也不经 ctx.sandbox，MCP 工具注册/执行路径上没有任何策略或审批门控。
  // 「受限会话不限制 MCP」是 DSH 现状，bgjobs 保持一致；requested 对 MCP 任务无意义，忽略。
  if (engine === 'mcp') return { mode: 'off', escalate: false }
