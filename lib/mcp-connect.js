...
/**
 * Node setTimeout 的上限（2^31−1 ms ≈ 24.8 天）：超过这个值的 delay 会被当成 1ms **立即触发**，
 * 所以所有"可能来自用户输入的毫秒数"都必须先过 clampTimerMs。
 */
export const MAX_TIMER_MS = 2147483647

/** 归一化超时（ms）：传正数则 clamp 到上限；未传/0/非法值 → 上限值（即「不限时」的具化值）。 */
export const clampTimerMs = (ms) => (Number.isFinite(ms) && ms > 0 ? Math.min(ms, MAX_TIMER_MS) : MAX_TIMER_MS)
